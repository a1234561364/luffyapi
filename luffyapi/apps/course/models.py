...
class Course(BaseModel):
    ...
    def section_list(self):
        ll = []
        # 根据课程取出所有章节(反向查询,related_name.all())
        course_chapter_list = self.coursechapters.all()
        for course_chapter in course_chapter_list:
            # 通过章节对象取到章节下所有的课程(反向查询)
            # course_chapter.表名小写_set.all() 现在变成了 course_chapter.coursesections.all()
